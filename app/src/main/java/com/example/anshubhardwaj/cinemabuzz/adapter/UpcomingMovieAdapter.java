...
public class UpcomingMovieAdapter extends RecyclerView.Adapter<UpcomingMovieAdapter.NowPlayingViewHolder> {
    ...
    @Override
    public void onBindViewHolder(UpcomingMovieAdapter.NowPlayingViewHolder viewHolder, int position) {

        MovieObject movieObject = movieList.get(position);

        Picasso.with(context).load(ApiClient.IMAGE_URL+movieObject.getPosterPath()).into(viewHolder.movieImage);
        viewHolder.tv_rating.setText(String.valueOf(movieObject.getVoteAverage()));
        viewHolder.tv_title.setText(movieObject.getTitle());

        String[] date = movieObject.getReleaseDate().split("-");
        viewHolder.tv_year.setText(String.valueOf(date[0]));

        try {
            Date d = new SimpleDateFormat("yyyy-MM-dd").parse(movieObject.getReleaseDate());
            Calendar calendar = new GregorianCalendar();
            calendar.setTime(d);
            viewHolder.tv_date.setText(calendar.getDisplayName(Calendar.MONTH, Calendar.LONG, Locale.ENGLISH )+" "+calendar.get(Calendar.DAY_OF_MONTH));
